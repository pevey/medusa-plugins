...
const prepareDeliveryCreatedNotificationStep = createStep('prepare-delivery-created-notification', async (id: string, { container }) => {
	...
	const {
		...
		entity: 'fulfillment',
		fields: ['id', 'order.id', 'items.quantity', 'items.line_item_id', 'veeqo_shipment.carrier', 'veeqo_shipment.tracking_number'],
		filters: { id }
	})
	...
	const orderItemMap = Object.fromEntries(((order as AnyRecord).items ?? []).map((item: AnyRecord) => [item.id, item]))
...
export const sendDeliveryCreatedNotificationWorkflow = createWorkflow('send-delivery-created-notification', (id: string) => {
