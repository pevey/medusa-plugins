...
const prepareFormSubmissionNotificationStep = createStep('prepare-form-submission-notification', async (id: string, { container }) => {
	...
	const formFields: AnyRecord[] = [...(sub.form?.form_fields ?? [])].sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0))
...
export const sendFormSubmissionNotificationWorkflow = createWorkflow('send-form-submission-notification', function (id: string) {
