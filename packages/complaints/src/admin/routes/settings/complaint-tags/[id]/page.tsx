...
export async function loader({ params }: LoaderFunctionArgs) {
	const { id } = params
	return sdk.client.fetch<ComplaintTagLoaderData>(`/admin/complaint-tags/${id}`, {
		query: { fields: 'id,value' }
	})
}

export const handle = {
	breadcrumb: ({ data }: UIMatch<ComplaintTagLoaderData>) => data?.complaint_tag?.value || data?.complaint_tag?.id || 'Complaint Tag'
}
...
const ComplaintTagDetailPage = () => {
	...
	const handleDelete = async () => {
		...
		if (confirmed) {
			deleteComplaintTags([complaintTag!.id], {
				onSuccess: () => {
					toast.success('Complaint tag deleted successfully')
					navigate('/settings/complaint-tags')
				},
	...
	if (!complaintTag)
		return (
			<Container className="p-6">
				<Text>Complaint tag not found.</Text>
			</Container>
		)
