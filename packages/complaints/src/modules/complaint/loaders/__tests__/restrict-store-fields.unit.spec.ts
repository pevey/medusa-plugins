...
describe('restrictStoreFields loader', () => {
	...
	it('declares the complaint relations restricted on /store by default', async () => {
		await restrictStoreFields({ options: undefined } as any)

		expect(registeredForStore()).toEqual(new Set(['complaint', 'complaints']))
	})
	...
	it('warns when adminOnly was set explicitly but the access plugin is absent', async () => {
		...
		expect(registeredForStore()).toBeUndefined()
		expect(warn).toHaveBeenCalledWith(expect.stringContaining('NOT hidden from store field expansion'))
	})
})
