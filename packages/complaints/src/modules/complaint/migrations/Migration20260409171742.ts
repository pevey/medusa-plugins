...
export class Migration20260409171742 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "complaint_tag" ("id" text not null, "value" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "complaint_tag_pkey" primary key ("id"));`
		)
		this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_complaint_tag_deleted_at" ON "complaint_tag" ("deleted_at") WHERE deleted_at IS NULL;`)

		this.addSql(
			`create table if not exists "complaint_complaint_tags" ("complaint_id" text not null, "complaint_tag_id" text not null, constraint "complaint_complaint_tags_pkey" primary key ("complaint_id", "complaint_tag_id"));`
		)

		this.addSql(`alter table if exists "complaint_activity" drop constraint if exists "complaint_activity_complaint_id_foreign";`)
		this.addSql(
			`alter table if exists "complaint_activity" add constraint "complaint_activity_complaint_id_foreign" foreign key ("complaint_id") references "complaint" ("id") on update cascade on delete cascade;`
		)
		...
		this.addSql(
			`alter table if exists "complaint_complaint_tags" add constraint "complaint_complaint_tags_complaint_id_foreign" foreign key ("complaint_id") references "complaint" ("id") on update cascade on delete cascade;`
		)
		...
		this.addSql(
			`alter table if exists "complaint_complaint_tags" add constraint "complaint_complaint_tags_complaint_tag_id_foreign" foreign key ("complaint_tag_id") references "complaint_tag" ("id") on update cascade on delete cascade;`
		)
	}
