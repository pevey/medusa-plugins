...
type Query = {
	graph: (args: { entity: string; fields: string[]; filters?: Record<string, unknown> }) => Promise<{ data: any[] }>
}
...
export async function loadComplaintsForExport(query: Query, ids: string[]): Promise<ComplaintForExport[]> {
	...
	const userIds = unique(complaints.flatMap((c: any) => (c.activity ?? []).map((a: any) => a.user_id).filter(Boolean)))
	...
	for (const c of complaints) {
		...
		complaintsById.set(c.id, {
			...
			order: orderRow ? { ...orderRow, created_at: new Date(orderRow.created_at) } : null,
