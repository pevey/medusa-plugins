...
export async function loader({ params }: LoaderFunctionArgs) {
	const { id } = params
	return sdk.client.fetch<CustomerTagLoaderData>(`/admin/customer-tags/${id}`, {
		query: { fields: 'id,value' }
	})
}

export const handle = {
	breadcrumb: ({ data }: UIMatch<CustomerTagLoaderData>) => data?.customer_tag?.value || data?.customer_tag?.id || 'Customer Tag'
}
...
const CustomerTagDetailPage = () => {
		...
		if (confirmed) {
			deleteCustomerTags([customerTag!.id], {
				onSuccess: () => {
					...
					navigate('/settings/customer-tags')
				},
	...
	if (!customerTag)
		return (
			<Container className="p-6">
				<Text>Customer tag not found.</Text>
			</Container>
		)
