...
import type { AdminCustomerTag } from '../types'
...
export function makeCustomerTag(overrides: Partial<AdminCustomerTag> = {}): AdminCustomerTag {
	return {
		id: 'ctag_1',
		value: 'VIP',
		...
		sdk: {
			client: {
				fetch: (path: string, options?: Parameters<ContractFake['fetch']>[1]) => {
					if (!currentFake) throw new Error('[customer-tags tests] no fake installed for this test')
