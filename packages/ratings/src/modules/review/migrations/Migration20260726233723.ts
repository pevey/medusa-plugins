...
export class Migration20260726233723 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "review_activity" add constraint "review_activity_type_check" check("type" in ('submit', 'approve', 'reject', 'feature', 'unfeature', 'note'));`
		)
	}
	...
	override async down(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "review_activity" add constraint "review_activity_type_check" check("type" in ('submit', 'approve', 'reject', 'note'));`
		)
	}
}
