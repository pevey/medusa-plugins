...
import { AutomationActionType, FieldMapping } from '../../../modules/automation/models/automation-action'
...
export const POST = async (req: MedusaRequest, res: MedusaResponse) => {
	...
	const results = await Promise.allSettled(
		actions.map(async action => {
			...
			let deliveryStatus = AutomationDeliveryStatus.PENDING
			...
			try {
				if (action.action_type === AutomationActionType.OUTGOING_WEBHOOK) {
					...
					if (fanoutMappings.length > 0) {
						...
						const maxIterOpt = automationService.getOptions().maxWorkflowIterations
						...
						if (iterErrors.length === 0) {
							deliveryStatus = AutomationDeliveryStatus.SUCCESS
						} else {
							deliveryStatus = AutomationDeliveryStatus.FAILED
	...
	const anyFailed = results.some(
		r =>
			r.status === 'rejected' ||
			(r.status === 'fulfilled' && r.value === AutomationDeliveryStatus.FAILED)
	)
