...
export class Migration20260409171930 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "automation_secret" ("id" text not null, "label" text not null, "secret" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "automation_secret_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "automation_receipt" ("id" text not null, "trigger_id" text not null, "request_ip" text null, "payload" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "automation_receipt_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "automation_action" ("id" text not null, "name" text not null, "description" text null, "is_active" boolean not null default true, "trigger_id" text not null, "action_type" text check ("action_type" in ('outgoing_webhook', 'outgoing_request', 'medusa_workflow')) not null, "target_url" text null, "signing_secret_id" text null, "request_method" text null, "target_headers" jsonb null, "medusa_workflow" text null, "field_mappings" jsonb null, "static_values" jsonb null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "automation_action_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`alter table if exists "automation_receipt" add constraint "automation_receipt_trigger_id_foreign" foreign key ("trigger_id") references "automation_trigger" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "automation_action" drop constraint if exists "automation_action_trigger_id_foreign";`)
		this.addSql(
			`alter table if exists "automation_action" add constraint "automation_action_trigger_id_foreign" foreign key ("trigger_id") references "automation_trigger" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "automation_query" drop constraint if exists "automation_query_action_id_foreign";`)
		this.addSql(
			`alter table if exists "automation_query" add constraint "automation_query_action_id_foreign" foreign key ("action_id") references "automation_action" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "automation_delivery" drop constraint if exists "automation_delivery_action_id_foreign";`)
		this.addSql(
			`alter table if exists "automation_delivery" add constraint "automation_delivery_action_id_foreign" foreign key ("action_id") references "automation_action" ("id") on update cascade on delete cascade;`
		)
	}
