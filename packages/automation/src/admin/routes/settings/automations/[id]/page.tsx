...
const AutomationTriggerDetailPage = () => {
	...
	const handleDelete = async () => {
		const confirmed = await prompt({
			...
			description: 'This will also delete all associated actions and deliveries. This action cannot be undone.',
			confirmText: 'Delete',
			cancelText: 'Cancel',
			variant: 'danger'
		})
	...
	return (
		<div className="flex flex-col gap-4 p-4">
			{showUnsignedWarning && (
				<Alert variant="warning">Treat this URL like a password. Without signing enabled, this URL will accept all incoming webhooks.</Alert>
			)}
			<Container className="divide-y p-0">
				...
				<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
					...
					<Badge size="xsmall" color={TRIGGER_COLORS[trigger.trigger_type]}>
				...
				<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
					...
					<Badge size="xsmall" color={trigger.is_active ? 'green' : 'grey'}>
					...
					<div className="text-ui-fg-subtle grid grid-cols-2 items-start px-6 py-4">
						...
						<div className="flex flex-wrap gap-1">
							{(trigger.trigger_events ?? []).length > 0 ? (
								trigger.trigger_events!.map(evt => (
				...
				{trigger.trigger_type === 'incoming_webhook' && (
					<>
						...
						<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
							...
							<Text size="small" leading="compact">
								{trigger.has_signing_key ? '••••••••' : 'Not set'}
							</Text>
						</div>
						<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
							<Text size="small" weight="plus" leading="compact">
							...
							<Badge size="xsmall" color={trigger.log_incoming ? 'green' : 'grey'}>
								{trigger.log_incoming ? 'Enabled' : 'Disabled'}
							</Badge>
						</div>
					</>
				)}
				...
				<Container className="divide-y p-0">
					<div className="px-6 py-4">
						...
						<Text size="small" className="text-ui-fg-subtle mt-1">
							Last 10 logged incoming payloads for this trigger.
						</Text>
					</div>
						...
						<div className="divide-y">
							{receiptsData.receipts.map(receipt => (
								<div key={receipt.id} className="grid grid-cols-3 items-start gap-2 px-6 py-3">
									<Text size="small" leading="compact" className="text-ui-fg-subtle col-span-1">
										{new Date(receipt.created_at).toLocaleString()}
									</Text>
									<Text size="small" leading="compact" className="text-ui-fg-subtle col-span-1">
										{receipt.request_ip ?? '—'}
									</Text>
									<pre className="bg-ui-bg-subtle col-span-1 overflow-x-auto rounded px-2 py-1 font-mono text-xs break-all whitespace-pre-wrap">
