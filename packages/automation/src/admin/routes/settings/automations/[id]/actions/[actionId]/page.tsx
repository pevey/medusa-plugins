...
export async function loader({ params }: LoaderFunctionArgs) {
	...
	return sdk.client.fetch<LoaderData>(`/admin/automations/${id}/actions/${actionId}`, {
...
const ACTION_LABELS: Record<ActionType, string> = {
	outgoing_webhook: 'Send Outgoing Webhook',
	outgoing_request: 'Send Outgoing Request',
	medusa_workflow: 'Run Medusa Workflow'
}
...
const DeliveryStatusBadge = ({ status }: { status: AutomationDelivery['status'] }) => {
	const color = status === 'success' ? 'green' : status === 'failed' ? 'red' : 'orange'
	return (
		<Badge size="xsmall" color={color}>
			{status}
		</Badge>
	)
}

const AutomationActionDetailPage = () => {
	const { id: triggerId, actionId } = useParams()
	const [editOpen, setEditOpen] = useState(false)
	const navigate = useNavigate()
	const prompt = usePrompt()

	const { data, isLoading } = useAutomationAction(triggerId, actionId)
	...
	const handleDelete = async () => {
		const confirmed = await prompt({
			title: 'Delete action?',
			description: 'This will also delete all associated deliveries. This action cannot be undone.',
			confirmText: 'Delete',
			cancelText: 'Cancel',
			variant: 'danger'
		})
		if (confirmed) {
			deleteActions([actionId!], {
				onSuccess: () => {
					...
					navigate(`/settings/automations/${triggerId}`)
				},
		...
		<div className="flex flex-col gap-4 p-4">
			<Container className="divide-y p-0">
				...
				<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
					...
					<Badge size="xsmall" color={ACTION_COLORS[action.action_type]}>
				...
				<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
					...
					<Badge size="xsmall" color={action.is_active ? 'green' : 'grey'}>
				...
				{!isOutgoing && (
					<div className="text-ui-fg-subtle grid grid-cols-2 items-center px-6 py-4">
						...
						<Text size="small" leading="compact">
							{action.medusa_workflow ? (MEDUSA_WORKFLOWS_BY_NAME[action.medusa_workflow]?.label ?? action.medusa_workflow) : '—'}
						</Text>
					</div>
				)}
			</Container>
				...
				<Container className="divide-y p-0">
					...
					<div className="px-6 py-4">
						<div className="mb-2 grid grid-cols-2 gap-2">
							...
							<Text size="small" weight="plus" leading="compact" className="text-ui-fg-subtle">
								{isOutgoing ? 'Outgoing Key' : 'Workflow Input'}
							</Text>
						</div>
						{action.field_mappings!.map((m, i) => (
							<div key={i} className="mt-2 grid grid-cols-2 gap-2">
								<Text size="small" leading="compact" className="font-mono text-xs">
									{m.source_path}
								</Text>
								<Text size="small" leading="compact" className="font-mono text-xs">
									{m.target_key}
				...
				<div className="flex items-center justify-between px-6 py-4">
					<div>
						...
						<Text size="small" className="text-ui-fg-subtle mt-1">
							Last 10 delivery attempts for this action.
						</Text>
					</div>
						...
						{deliveriesData.deliveries.map(delivery => (
							<div key={delivery.id} className="grid grid-cols-[auto_1fr_auto_1fr_1fr] items-center gap-2 px-6 py-3">
								<input
									...
									className="border-ui-border-base rounded"
								/>
								<Text size="small" leading="compact" className="truncate font-mono text-xs">
									{delivery.event_name}
								</Text>
			...
			{action && <EditAutomationActionDrawer action={action} triggerId={triggerId!} open={editOpen} setOpen={setEditOpen} />}
		</div>
	)
}

export default AutomationActionDetailPage
