...
export async function loader({ params }: LoaderFunctionArgs) {
	...
	return sdk.client.fetch<LoaderData>(`/admin/automations/${id}/actions/${actionId}`, { query: { fields: 'id,name' } })
}
...
const AutomationActionDetailPage = () => {
	...
	const handleDelete = async () => {
		...
		if (confirmed) {
			deleteActions([actionId!], {
				onSuccess: () => { toast.success('Action deleted'); navigate(`/settings/automations/${triggerId}`) },
	...
	return (
		<div className="flex flex-col gap-4 p-4">
			...
			{action && (
				<EditAutomationActionDrawer action={action} triggerId={triggerId!} open={editOpen} setOpen={setEditOpen} />
			)}
		</div>
	)
}
