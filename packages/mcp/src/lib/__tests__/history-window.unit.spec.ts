...
const toolTurn = (q: string, id: string): ChatMessage[] => [
	userText(q),
	{ role: 'assistant', content: [{ type: 'text', text: 'let me check' }, { type: 'tool_use', id, name: 'get', input: {} }] },
...
describe('dropUnpairedToolUse', () => {
	it('drops a tool_use with no following tool_result, keeping the message\'s text', () => {
		const msgs: ChatMessage[] = [
			userText('q'),
			{ role: 'assistant', content: [{ type: 'text', text: 'let me check' }, { type: 'tool_use', id: 'dangling', name: 'get', input: {} }] },
		]
		expect(dropUnpairedToolUse(msgs)).toEqual([
			userText('q'),
	...
	it('drops the whole message when it would be left empty (tool_use-only)', () => {
		const msgs: ChatMessage[] = [
			userText('q'),
			{ role: 'assistant', content: [{ type: 'tool_use', id: 'dangling', name: 'get', input: {} }] },
		]
