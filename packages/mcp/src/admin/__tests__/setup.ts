...
// be visible here — `middlewares.ts` only supplies a schema for `POST /admin/chat` (its only
// wired route; see contract-invariants.test.ts). Read as raw text, NOT executed: a route.ts
// pulls in `@medusajs/framework/utils` -> `jsonwebtoken`, which calls `util.inherits` and
// crashes the whole suite import once Vite externalizes Node's `util` for the browser.
		...
		sdk: {
			client: {
				fetch: (path: string, options?: Parameters<ContractFake['fetch']>[1]) => {
					if (!currentFake) throw new Error('[mcp tests] no fake installed for this test')
