...
import { ShippingOptionForVeeqoDeliveryMethodInput, VeeqoDeliveryMethodDTO, VeeqoDeliveryMethodInput } from '../modules/veeqo/types'

const mapShippingOptionToVeeqoDeliveryMethodInput = (shipping_option: ShippingOptionForVeeqoDeliveryMethodInput): VeeqoDeliveryMethodInput => {
...
export const getShippingOptionDetailsStep = createStep('get-shipping-option-details-step', async (shipping_option_id: string, { container }) => {
	...
	const shipping_option = shipping_options[0] as ShippingOptionForVeeqoDeliveryMethodInput | undefined
	if (!shipping_option) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `Shipping option with id ${shipping_option_id} not found`)
	}
...
export const fetchDeliveryMethodFromVeeqoStep = createStep('fetch-delivery-method-from-veeqo-step', async (shipping_option_id: string, { container }) => {
	const query = container.resolve('query')
	const {
	...
	if (!veeqoDeliveryMethodId) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo delivery method found for shipping option ${shipping_option_id}`)
	}
	const veeqoService = container.resolve('veeqo') as VeeqoService
	const veeqoDeliveryMethod = (await veeqoService.fetchDeliveryMethod(veeqoDeliveryMethodId)) as VeeqoDeliveryMethodDTO
	return new StepResponse(veeqoDeliveryMethod)
})

export const getVeeqoDeliveryMethodWorkflow = createWorkflow('get-veeqo-delivery-method-workflow', (shipping_option_id: string) => {
...
export const addDeliveryMethodToVeeqoWorkflow = createWorkflow('add-delivery-method-to-veeqo-workflow', (shipping_option_id: string) => {
...
export const updateDeliveryMethodInVeeqoStep = createStep(
	...
	async (shipping_option: ShippingOptionForVeeqoDeliveryMethodInput, { container }) => {
		...
		if (!veeqoDeliveryMethodId) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo delivery method found for shipping option ${shipping_option.id}`)
		}
		const veeqoDeliveryMethodInput = mapShippingOptionToVeeqoDeliveryMethodInput(shipping_option)
		const veeqoService: VeeqoService = container.resolve('veeqo')
...
export const updateDeliveryMethodInVeeqoWorkflow = createWorkflow('update-delivery-method-in-veeqo-workflow', (shipping_option_id: string) => {
	const shippingOption = getShippingOptionDetailsStep(shipping_option_id)
...
export const syncDeliveryMethodToVeeqoWorkflow = createWorkflow('sync-delivery-method-to-veeqo-workflow', (shipping_option_id: string) => {
	const shippingOption = getShippingOptionDetailsStep(shipping_option_id)
...
export const deleteDeliveryMethodFromVeeqoWorkflow = createWorkflow('delete-delivery-method-from-veeqo-workflow', (shipping_option_id: string) => {
