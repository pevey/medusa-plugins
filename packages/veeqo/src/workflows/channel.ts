...
import { SalesChannelForVeeqoChannelInput, VeeqoChannelDTO, VeeqoChannelInput } from '../modules/veeqo/types'

const mapSalesChannelToVeeqoChannelInput = (sales_channel: SalesChannelForVeeqoChannelInput): VeeqoChannelInput => {
	if (sales_channel.name === undefined || sales_channel.name === null) {
		throw new MedusaError(MedusaError.Types.INVALID_DATA, `Sales channel ${sales_channel.id} is missing required field for Veeqo channel input: name`)
	}
	return {
...
export const getSalesChannelDetailsStep = createStep('get-sales-channel-details-step', async (sales_channel_id: string, { container }) => {
	...
	if (!sales_channel) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `Sales channel with id ${sales_channel_id} not found`)
	}
...
export const fetchChannelFromVeeqoStep = createStep('fetch-channel-from-veeqo-step', async (sales_channel_id: string, { container }) => {
	const query = container.resolve('query')
	const {
		data: [veeqo_channel]
	} = await query.graph({
	...
	if (!veeqoChannelId) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo channel found for sales channel ${sales_channel_id}`)
	}
...
export const getVeeqoChannelWorkflow = createWorkflow('get-veeqo-channel-workflow', (sales_channel_id: string) => {
...
export const addChannelToVeeqoStep = createStep('add-channel-to-veeqo-step', async (sales_channel: SalesChannelForVeeqoChannelInput, { container }) => {
	...
	if (!veeqoChannel) {
		throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Unable to create channel in Veeqo for sales channel ${sales_channel.id}`)
	}
	return new StepResponse(veeqoChannel)
})

export const addSalesChannelToVeeqoWorkflow = createWorkflow('add-sales-channel-to-veeqo-workflow', (sales_channel: SalesChannelForVeeqoChannelInput) => {
...
export const updateChannelInVeeqoStep = createStep('update-channel-in-veeqo-step', async (sales_channel: SalesChannelForVeeqoChannelInput, { container }) => {
	if (!sales_channel.veeqo_channel?.veeqo_channel_id) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo channel found for sales channel ${sales_channel.id}`)
	}
	const veeqoChannelInput = mapSalesChannelToVeeqoChannelInput(sales_channel)
	const veeqoService: VeeqoService = container.resolve('veeqo')
	const veeqoChannel = await veeqoService.updateChannel(sales_channel.veeqo_channel.veeqo_channel_id, veeqoChannelInput)
	if (!veeqoChannel) {
		throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Unable to update channel in Veeqo for sales channel ${sales_channel.id}`)
	}
	return new StepResponse(veeqoChannel)
})

export const updateSalesChannelInVeeqoWorkflow = createWorkflow('update-sales-channel-in-veeqo-workflow', (sales_channel: SalesChannelForVeeqoChannelInput) => {
...
export const syncChannelToVeeqoWorkflow = createWorkflow('sync-channel-in-veeqo-workflow', (sales_channel_id: string) => {
...
export const findAndDeleteChannelInVeeqoStep = createStep('find-and-delete-channel-in-veeqo-step', async (sales_channel_id: string, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	await veeqoService.deleteChannel(sales_channel_id)
	return new StepResponse(void 0)
})

export const deleteChannelInVeeqoWorkflow = createWorkflow('delete-channel-in-veeqo-workflow', (sales_channel_id: string) => {
