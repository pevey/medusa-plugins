...
const fetchVeeqoOrderDataStep = createStep('fetch-veeqo-order-data-step', async (input: SyncInput, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	const [dbOrder] = await veeqoService.listVeeqoOrders({ id: input.veeqoOrderDbId }, { relations: ['veeqo_shipments'] })
	...
	if (dbOrder.veeqo_order_id == null) {
		throw new Error(`VeeqoOrder ${input.veeqoOrderDbId} has no veeqo_order_id (placeholder row from a failed create attempt)`)
	}
...
const processNewShipmentsStep = createStep('process-new-veeqo-shipments-step', async (data: SyncData, { container }) => {
...
const completeOrderIfShippedStep = createStep('complete-order-if-shipped-step', async (data: SyncData, { container }) => {
	const { dbOrder } = data
	const veeqoService: VeeqoService = container.resolve('veeqo')
...
const processDeliveredShipmentsStep = createStep('process-delivered-veeqo-shipments-step', async (data: SyncData, { container }) => {
	const { dbOrder } = data
	const veeqoService: VeeqoService = container.resolve('veeqo')
...
const finalizeVeeqoOrderSyncStep = createStep('finalize-veeqo-order-sync-step', async (data: SyncData, { container }) => {
	const { dbOrder } = data
	const veeqoService: VeeqoService = container.resolve('veeqo')
	...
	const allDelivered = currentShipments.length > 0 && currentShipments.every((s: any) => s.veeqo_tracking_events != null)
...
export const syncVeeqoOrderShipmentsWorkflow = createWorkflow('sync-veeqo-order-shipments-workflow', function (input: SyncInput) {
