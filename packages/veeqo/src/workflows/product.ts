...
import { ProductForVeeqoProductInput, ProductVariantForVeeqoProductInput, VeeqoProductDTO, VeeqoProductInput } from '../modules/veeqo/types'
...
const mapVariantToVeeqoVariantInput = (
	...
	return {
		...(veeqoSellableId !== undefined && veeqoSellableId !== null ? { id: Number(veeqoSellableId) } : {}),
...
const mapProductToVeeqoProductInput = (product: ProductForVeeqoProductInput): VeeqoProductInput => {
	...
	if (!requiredFields.variants!.length) {
		throw new MedusaError(MedusaError.Types.INVALID_DATA, `Product ${product.id} must include at least one variant for Veeqo product input`)
	}
	...
	return {
		...
		product_variants_attributes: requiredFields.variants!.map((variant, index) => mapVariantToVeeqoVariantInput(product, variant, index))
	}
}

export const getProductDetailsStep = createStep('get-product-details-step', async (productId: string, { container }) => {
...
export const fetchProductFromVeeqoStep = createStep('fetch-product-from-veeqo-step', async (product_id: string, { container }) => {
	const query = container.resolve('query')
	const {
		data: [veeqo_product]
	} = await query.graph({
	...
	if (!veeqoProductId) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo product found for product ${product_id}`)
	}
...
export const getVeeqoProductWorkflow = createWorkflow('get-veeqo-product-workflow', (product_id: string) => {
...
export const addProductToVeeqoStep = createStep('add-product-to-veeqo-step', async (product: ProductForVeeqoProductInput, { container }) => {
...
export const addProductToVeeqoWorkflow = createWorkflow('add-product-to-veeqo-workflow', (product_id: string) => {
...
export const updateProductInVeeqoStep = createStep('update-product-in-veeqo-step', async (product: ProductForVeeqoProductInput, { container }) => {
	const veeqoProductId = product.veeqo_product?.veeqo_product_id
	if (!veeqoProductId) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo product found for product ${product.id}`)
	}
	const veeqoProductInput = mapProductToVeeqoProductInput(product)
	const veeqoService: VeeqoService = container.resolve('veeqo')
...
const updateProductInVeeqoWorkflow = createWorkflow('update-product-in-veeqo-workflow', (product_id: string) => {
	const product = getProductDetailsStep(product_id)
...
export const addOrUpdateProductInVeeqoStep = createStep('add-or-update-product-in-veeqo-step', async (product: ProductForVeeqoProductInput, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	const veeqoProductId = product.veeqo_product?.veeqo_product_id
	if (!veeqoProductId) {
		const veeqoProductInput = mapProductToVeeqoProductInput(product)
...
export const syncProductToVeeqoWorkflow = createWorkflow('sync-product-to-veeqo-workflow', (product_id: string) => {
	const product = getProductDetailsStep(product_id)
...
export const findAndDeleteProductFromVeeqoStep = createStep('find-and-delete-product-from-veeqo-step', async (product_id: string, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	await veeqoService.deleteProduct(product_id)
	return new StepResponse(void 0)
})

const deleteProductFromVeeqoWorkflow = createWorkflow('delete-product-from-veeqo-workflow', (product_id: string) => {
