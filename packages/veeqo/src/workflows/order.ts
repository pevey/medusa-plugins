...
import { OrderForVeeqoOrderInput, VeeqoAddress, VeeqoOrderDTO, VeeqoOrderInput } from '../modules/veeqo/types'
...
const mapOrderToVeeqoOrderInput = (order: OrderForVeeqoOrderInput, veeqoCustomerId: number): VeeqoOrderInput => {
	...
	const deliveryMethodId = order.shipping_methods?.[0]?.shipping_option?.veeqo_delivery_method?.veeqo_delivery_method_id
	if (!deliveryMethodId) missingFields.push('shipping_methods[0].shipping_option.veeqo_delivery_method.veeqo_delivery_method_id')

	if (!order.items?.length) missingFields.push('items')

	const itemsMissingSellable = (order.items ?? []).filter(item => !item.variant?.veeqo_sellable?.veeqo_sellable_id)
	if (itemsMissingSellable.length) {
		missingFields.push(`items[${itemsMissingSellable.map(item => item.id).join(', ')}].variant.veeqo_sellable.veeqo_sellable_id`)
	}

	if (missingFields.length) {
		throw new MedusaError(MedusaError.Types.INVALID_DATA, `Order ${order.id} is missing required fields for Veeqo order input: ${missingFields.join(', ')}`)
	}
	...
	return {
		...
		payment_attributes: {
			payment_type: order.payment_collections?.[0]?.payment_sessions?.[0]?.provider_id ?? 'manual',
...
const buildVeeqoOrderNumber = (sourceType: SourceType, orderId: string, sourceId: string): string => {
	switch (sourceType) {
...
const mapReplacementContextToVeeqoOrderInput = (ctx: ReplacementContext, veeqoCustomerId: number): VeeqoOrderInput | null => {
	...
	if (!ctx.shipping_method?.veeqo_delivery_method_id) missingFields.push('shipping_method.veeqo_delivery_method_id')

	const itemsMissingSellable = ctx.additional_items.filter(item => !item.variant_id || !ctx.variants_with_sellable[item.variant_id]?.veeqo_sellable_id)
	if (itemsMissingSellable.length) {
		missingFields.push(`additional_items[${itemsMissingSellable.map(i => i.id).join(', ')}].variant.veeqo_sellable_id`)
	}
...
export const findVeeqoOrderBySourceStep = createStep(
	...
	async (input: { source_type: SourceType; source_id: string }, { container }) => {
...
export const createVeeqoOrderStep = createStep(
	...
	async (
		...
		if (input.veeqo_input == null) {
			logger.info(`veeqo: skipping ${input.source_type} ${input.source_id} — no items to ship`)
		...
		if (existing && (existing as any).veeqo_order_id != null && (existing as any).last_sync_error == null) {
			logger.info(`veeqo: ${input.source_type} ${input.source_id} already synced (veeqo_order_id=${(existing as any).veeqo_order_id})`)
			return new StepResponse(null)
		}
		...
		try {
			...
			if (!apiResponse?.id) {
				throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Veeqo addOrder returned no id for ${input.source_type} ${input.source_id}`)
			}
			...
			logger.warn(`veeqo: failed to sync ${input.source_type} ${input.source_id} (medusa order ${input.order_id}): ${message}`)
...
export const getOrderDetailsStep = createStep('get-medusa-order-details-step', async (order_id: string, { container }) => {
	...
	const shippingOptionIds = orderShippingMethods.map(sm => sm.shipping_option_id as string).filter(Boolean)
	...
	const [{ data: customers }, { data: salesChannels }, variantMap, shippingOptionMap] = await Promise.all([
		query.graph({
			entity: 'customer',
		...
		variantIds.length > 0
			? query
					...
					.then(({ data: variants }) => Object.fromEntries((variants as AnyRecord[]).map(v => [v.id, v])))
		...
		shippingOptionIds.length > 0
			? query
					...
					.then(({ data: shippingOptions }) => Object.fromEntries((shippingOptions as AnyRecord[]).map(so => [so.id, so])))
			: Promise.resolve({} as Record<string, AnyRecord>)
	])
	...
	const result: OrderForVeeqoOrderInput = {
		...
		customer: customer0
			? {
					...
					veeqo_customer: customer0.veeqo_customer ? { veeqo_customer_id: customer0.veeqo_customer.veeqo_customer_id } : undefined
				}
			: null,
		...
		sales_channel: sc0
			? {
					id: sc0.id,
					veeqo_channel: sc0.veeqo_channel ? { veeqo_channel_id: sc0.veeqo_channel.veeqo_channel_id } : undefined
				}
			: null,
...
export const fetchOrderFromVeeqoStep = createStep(
	...
	async (input: { source_type: SourceType; source_id: string }, { container }) => {
		...
		if (!veeqoOrderId) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `No linked Veeqo order found for ${input.source_type} ${input.source_id}`)
		}
...
export const getVeeqoOrderWorkflow = createWorkflow('get-veeqo-order-workflow', (input: { source_type: SourceType; source_id: string }) => {
...
export const addOrderToVeeqoWorkflow = createWorkflow('add-order-to-veeqo-workflow', (order_id: string) => {
	...
	const veeqoInput = transform({ order, veeqoCustomer }, ({ order, veeqoCustomer }) => mapOrderToVeeqoOrderInput(order, (veeqoCustomer as any)?.id as number))
...
export const updateOrderInVeeqoStep = createStep('update-order-in-veeqo-step', async (input: { source_type: SourceType; source_id: string }, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	const [existing] = await veeqoService.listVeeqoOrders({
		source_type: input.source_type,
		source_id: input.source_id
...
export const updateOrderInVeeqoWorkflow = createWorkflow('update-order-in-veeqo-workflow', (input: { source_type: SourceType; source_id: string }) => {
...
export const refreshExistingPlacedVeeqoOrderStep = createStep(
	...
	async (input: { order_id: string }, { container }) => {
		...
		const veeqoOrder = (await veeqoService.fetchOrder((dbOrder as any).veeqo_order_id)) as VeeqoOrderDTO

		await syncVeeqoOrderShipmentsWorkflow(container)
			.run({
				input: { veeqoOrderDbId: (dbOrder as any).id as string }
			})
			.catch((err: Error) => container.resolve('logger').warn(`veeqo: failed to sync shipment status for order ${input.order_id}: ${err.message}`))
 ...
 */
export const syncOrderToVeeqoWorkflow = createWorkflow('sync-order-to-veeqo-workflow', (order_id: string) => {
	const order = getOrderDetailsStep(order_id)
	...
	const refreshed = refreshExistingPlacedVeeqoOrderStep(transform(order, o => ({ order_id: o.id })))
...
export const getReplacementContextStep = createStep(
	...
	async (input: { order_id: string; source_type: SourceType; source_id: string }, { container }) => {
		const query = container.resolve('query')

		// 1. Fetch the claim or exchange + its additional_items (line items to ship)
		const sourceEntity = input.source_type === SourceType.CLAIM ? 'order_claim' : 'order_exchange'
		...
		if (!sourceRow) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `${input.source_type} ${input.source_id} not found`)
		}

		const additionalItems: ReplacementItem[] = ((sourceRow.additional_items ?? []) as any[]).map(ci => ({
			id: ci.id as string,
		...
		if (!orderRow) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Order ${input.order_id} not found for ${input.source_type} ${input.source_id}`)
		}

		// 3. Fetch cross-module data in parallel
		const variantIds = additionalItems.map(i => i.variant_id).filter((v): v is string => Boolean(v))
		const shippingOptionIds = ((orderRow.shipping_methods ?? []) as any[]).map(sm => sm.shipping_option_id as string).filter(Boolean)

		const [parentVeeqoRows, customerRes, salesChannelRes, variantsWithSellable, shippingOptionRes] = await Promise.all([
			query.graph({
				entity: 'veeqo_order',
		...
		const result: ReplacementContext = {
			...
			customer: customer ? { id: customer.id, email: customer.email ?? null, phone: customer.phone ?? null } : null,
			...
			shipping_method: shippingOption
				? {
						...
						veeqo_delivery_method_id: shippingOption.veeqo_delivery_method?.veeqo_delivery_method_id
					}
				: null,
...
export const syncReplacementToVeeqoWorkflow = createWorkflow(
	...
	(input: { orderId: string; sourceType: SourceType; sourceId: string }) => {
		...
		const veeqoInput = transform({ ctx, veeqoCustomer }, ({ ctx, veeqoCustomer }) =>
...
export const findAndDeleteOrderFromVeeqoStep = createStep('find-and-delete-order-from-veeqo-step', async (order_id: string, { container }) => {
	const veeqoService: VeeqoService = container.resolve('veeqo')
	await veeqoService.deleteOrder(order_id)
	return new StepResponse(void 0)
})

const deleteOrderFromVeeqoWorkflow = createWorkflow('delete-order-from-veeqo-workflow', (order_id: string) => {
