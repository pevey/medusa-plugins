...
export class VeeqoService extends MedusaService({
	...
	private async fetch(options: FetchOptions, retry: number = this.options_.retry || 1): Promise<Response> {
		...
		try {
			...
			if (retry > 0) {
				...
				this.logger_.warn(`[VEEQO]: Veeqo API request failed. Retrying in ${backoffTime}ms... (${retry} retries left)`)
	...
	async fetchChannel(veeqoChannelId: number): Promise<VeeqoChannelDTO> {
		return this.fetch({
			...
			.catch(err => {
				...
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo channel with veeqo_channel_id ${veeqoChannelId} not found`)
			})
	}
	...
	async fetchCustomer(veeqoCustomerId: number): Promise<VeeqoCustomerDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo customer with veeqo_customer_id ${veeqoCustomerId} not found`)
			})
	}
	...
	async fetchDeliveryMethod(veeqoDeliveryMethodId: number): Promise<VeeqoDeliveryMethodDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo delivery method with veeqo_delivery_method_id ${veeqoDeliveryMethodId} not found`)
			})
	}
	...
	async fetchOrder(veeqoOrderId: number): Promise<VeeqoOrderDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo order with veeqo_order_id ${veeqoOrderId} not found`)
			})
	}
	...
	async fetchProduct(veeqoProductId: number): Promise<VeeqoProductDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo product with veeqo_product_id ${veeqoProductId} not found`)
			})
	}
	...
	async fetchShipment(shipmentId: number): Promise<VeeqoShipmentDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo shipment with veeqo_shipment_id ${shipmentId} not found`)
			})
	}
	...
	async fetchTrackingEvents(shipmentId: number): Promise<VeeqoTrackingEventDTO[]> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Tracking events for shipment with veeqo_shipment_id ${shipmentId} not found`)
			})
	}
	...
	async fetchWarehouse(warehouseId: number): Promise<VeeqoWarehouseDTO> {
		return await this.fetch({
			...
			.catch(err => {
				this.logger_.error(err)
				throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo warehouse with veeqo_warehouse_id ${warehouseId} not found`)
			})
	}
	 ...
	 */
	async addChannel(salesChannelId: string, channelInput: VeeqoChannelInput): Promise<VeeqoChannelDTO> {
		...
		if (!veeqoChannel?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create channel in Veeqo for sales channel ${salesChannelId}`)
		}
	 ...
	 */
	async addCustomer(customerId: string, customerInput: VeeqoCustomerInput): Promise<VeeqoCustomerDTO> {
		...
		if (!veeqoCustomer?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create customer in Veeqo for customer ${customerId}`)
		}
	 ...
	 */
	async addDeliveryMethod(shippingOptionId: string, deliveryMethodInput: VeeqoDeliveryMethodInput): Promise<VeeqoDeliveryMethodDTO> {
		...
		if (!veeqoDeliveryMethod?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create delivery method in Veeqo for shipping option ${shippingOptionId}`)
		}
	...
	async addOrder(input: { veeqo_input: VeeqoOrderInput }): Promise<VeeqoOrderDTO> {
		...
		if (!response.ok || !body?.id) {
			...
			this.logger_.error(`veeqo addOrder failed (number: ${input.veeqo_input.number}): ${detail}`)
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create order in Veeqo (number: ${input.veeqo_input.number}): ${detail}`)
		}
	 ...
	 */
	async addProduct(product: ProductForVeeqoProductInput, productInput: VeeqoProductInput): Promise<VeeqoProductDTO> {
		...
		if (!veeqoProduct?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create product in Veeqo for product ${product.id}`)
		}
		...
		for (const sellable of veeqoProduct.sellables) {
			const matchingVariant = product.variants?.find(variant => variant.sku === sellable.sku_code)
			if (matchingVariant) {
	 ...
	 */
	async addWarehouse(stockLocationId: string, warehouseInput: VeeqoWarehouseInput): Promise<VeeqoWarehouseDTO> {
		...
		if (!veeqoWarehouse?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to create warehouse in Veeqo for stock location ${stockLocationId}`)
		}
	...
	async updateChannel(veeqoChannelId: number, veeqoChannelInput: VeeqoChannelInput): Promise<VeeqoChannelDTO> {
		...
		if (!veeqoChannel?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update channel in Veeqo with Veeqo ID ${veeqoChannelId}`)
		}
		return veeqoChannel
	}
	...
	async updateCustomer(veeqoCustomerId: number, veeqoCustomerInput: VeeqoCustomerInput): Promise<VeeqoCustomerDTO> {
		...
		if (!veeqoCustomer?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update customer in Veeqo with Veeqo ID ${veeqoCustomerId}`)
		}
		return veeqoCustomer
	}
	...
	async updateDeliveryMethod(veeqoDeliveryMethodId: number, veeqoDeliveryMethodInput: VeeqoDeliveryMethodInput): Promise<VeeqoDeliveryMethodDTO> {
		...
		if (!veeqoDeliveryMethod?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update delivery method in Veeqo with Veeqo ID ${veeqoDeliveryMethodId}`)
		}
		return veeqoDeliveryMethod
	}
	...
	async updateOrder(veeqoOrderId: number, veeqoOrderInput: VeeqoOrderInput): Promise<VeeqoOrderDTO> {
		...
		if (!veeqoOrder?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update order in Veeqo with Veeqo ID ${veeqoOrderId}`)
		}
		return veeqoOrder
	}
	...
	async updateProduct(veeqoProductId: number, veeqoProductInput: VeeqoProductInput): Promise<VeeqoProductDTO> {
		...
		if (!veeqoProduct?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update product in Veeqo with Veeqo ID ${veeqoProductId}`)
		}
		const needStockEntryUpdate = veeqoProduct.sellables?.filter(sellable => {
			sellable.inventory.infinite === false
		})
		if (needStockEntryUpdate?.length) await this.addInfiniteStockForSellables(needStockEntryUpdate)
		return veeqoProduct
	}
	...
	async updateWarehouse(veeqoWarehouseId: number, veeqoWarehouseInput: VeeqoWarehouseInput): Promise<VeeqoWarehouseDTO> {
		...
		if (!veeqoWarehouse?.id) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to update warehouse in Veeqo with Veeqo ID ${veeqoWarehouseId}`)
		}
		return veeqoWarehouse
	}
	...
	async deleteChannel(salesChannelId: string): Promise<void> {
		...
		if (!veeqoChannel) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo channel linked to sales channel ${salesChannelId} not found`)
		}
		...
		if (!success) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to delete channel in Veeqo with Veeqo ID ${veeqo_channel_id}`)
		}
	...
	async deleteDeliveryMethod(shippingOptionId: string): Promise<void> {
		...
		if (!veeqoDeliveryMethod) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo delivery method linked to shipping option ${shippingOptionId} not found`)
		}
		...
		const success = await this.fetch({
			...
			.catch(err => this.logger_.error(err))
		if (!success) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to delete delivery method in Veeqo with Veeqo ID ${veeqo_delivery_method_id}`)
		}
	...
	async deleteOrder(orderId: string, cancelReason: string = ''): Promise<void> {
		...
		if (!veeqoOrder) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo order linked to order ${orderId} not found`)
		}
		...
		const success = await this.fetch({
			...
			.catch(err => this.logger_.error(err))
		if (!success) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to cancel order in Veeqo with Veeqo ID ${veeqo_order_id}`)
		}
	...
	async deleteProduct(productId: string): Promise<void> {
		...
		if (!veeqoProduct) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo product linked to product ${productId} not found`)
		}
		...
		const success = await this.fetch({
			...
			.catch(err => this.logger_.error(err))
		if (!success) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to delete product in Veeqo with Veeqo ID ${veeqo_product_id}`)
		}
	...
	async deleteWarehouse(stockLocationId: string): Promise<void> {
		...
		if (!veeqoWarehouse) {
			throw new MedusaError(MedusaError.Types.NOT_FOUND, `Veeqo warehouse linked to stock location ${stockLocationId} not found`)
		}
		...
		const success = await this.fetch({
			...
			.catch(err => this.logger_.error(err))
		if (!success) {
			throw new MedusaError(MedusaError.Types.UNEXPECTED_STATE, `Failed to delete warehouse in Veeqo with Veeqo ID ${veeqo_warehouse_id}`)
		}
