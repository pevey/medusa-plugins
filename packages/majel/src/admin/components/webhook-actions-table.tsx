...
import { useWebhookActions, useDeleteWebhookActions } from '../hooks/webhooks'
import {
	Badge,
	Button,
	Container,
...
import { CreateWebhookActionModal } from './create-webhook-action-modal'
import { ActionType, WebhookAction } from '../types'
...
const columnHelper = createDataTableColumnHelper<WebhookAction>()

export const WebhookActionsTable = ({ triggerId, triggerType, triggerEvents = [] }: Props) => {
	const limit = 10
	...
	const { data, isLoading } = useWebhookActions(triggerId, { limit, offset })
	const { mutateAsync: deleteActions } = useDeleteWebhookActions(triggerId)
