...
export default async function searchCategoryHandler({
	...
	try {
		if (eventName === 'product-category.deleted') {
			await deleteSearchDocumentWorkflow(container).run({ input: { type: 'category', id: data.id } })
		} else {
			await upsertSearchDocumentWorkflow(container).run({ input: { type: 'category', id: data.id } })
		}
	} catch (error) {
