...
export class Migration20260713063739 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "search_document" ("id" text not null, "type" text not null, "entity_id" text not null, "slug" text not null, "group_slug" text null, "title" text not null, "snippet" text null, "primary_text" text not null, "secondary_text" text null, "weight" integer not null default 1, "sales_channel_ids" text[] null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "search_document_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_search_document_type_entity_id_unique" ON "search_document" ("type", "entity_id") WHERE deleted_at IS NULL;`
		)
