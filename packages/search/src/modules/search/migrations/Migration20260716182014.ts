...
export class Migration20260716182014 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`alter table if exists "search_document_translation" drop constraint if exists "search_document_translation_search_document_id_locale_unique";`
		)
		this.addSql(
			`create table if not exists "search_document_translation" ("id" text not null, "locale" text not null, "title" text not null, "snippet" text null, "primary_text" text not null, "body_text" text null, "search_document_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "search_document_translation_pkey" primary key ("id"));`
		)
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_search_document_translation_search_document_id" ON "search_document_translation" ("search_document_id") WHERE deleted_at IS NULL;`
		)
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_search_document_translation_deleted_at" ON "search_document_translation" ("deleted_at") WHERE deleted_at IS NULL;`
		)
		this.addSql(
			`CREATE UNIQUE INDEX IF NOT EXISTS "IDX_search_document_translation_search_document_id_locale_unique" ON "search_document_translation" ("search_document_id", "locale") WHERE deleted_at IS NULL;`
		)

		// deleting a base document prunes its translations
		this.addSql(
			`alter table if exists "search_document_translation" add constraint "search_document_translation_search_document_id_foreign" foreign key ("search_document_id") references "search_document" ("id") on update cascade on delete cascade;`
		)
