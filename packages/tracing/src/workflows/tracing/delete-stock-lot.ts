...
const calculateAdjustmentStep = createStep('calculate-adjustment', async (input: { id: string }, { container }) => {
...
const deleteStockLotStep = createStep('delete-stock-lot', async (input: { id: string }, { container }) => {
	const tracingService: TracingService = container.resolve(TRACING_MODULE)
...
export const deleteStockLotWorkflow = createWorkflow('delete-stock-lot', (input: { id: string }) => {
