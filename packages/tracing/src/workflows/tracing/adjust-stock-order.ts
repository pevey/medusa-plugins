...
const adjustStockLotStep = createStep('adjust-stock-lot', async ({ order_id }: AdjustStockLotStepInput, { container }) => {
	const tracingService: TracingService = container.resolve(TRACING_MODULE)

	const query = container.resolve('query')
	const { data: orders } = await query.graph({
		entity: 'order',
		fields: ['id', 'items.*', 'items.variant.*', 'items.variant.inventory.*', 'items.variant.inventory.location_levels.*'],
		filters: { id: order_id }
	})
...
export const adjustStockOrderWorkflow = createWorkflow('adjust-stock-order', ({ order_id }: AdjustStockOrderWorkflowInput) => {
