...
const calculateAdjustmentStep = createStep('calculate-adjustment', async (input: AdminUpdateStockLotType & { id: string }, { container }) => {
	...
	if (input.enabled !== undefined && input.enabled !== currentStockLot.enabled) {
		adjustment += input.enabled ? currentStockLot.stocked_quantity : -currentStockLot.stocked_quantity
	} else if (input.stocked_quantity !== undefined && input.stocked_quantity !== currentStockLot.stocked_quantity) {
		adjustment = input.stocked_quantity - currentStockLot.stocked_quantity
	}
	return new StepResponse(adjustment)
})

const updateStockLotStep = createStep('update-stock-lot', async (input: AdminUpdateStockLotType & { id: string }, { container }) => {
	const tracingService: TracingService = container.resolve(TRACING_MODULE)
	const stockLot = await tracingService.updateStockLots(input)
	return new StepResponse(stockLot, stockLot.id)
})

export const updateStockLotWorkflow = createWorkflow('update-stock-lot', (input: AdminUpdateStockLotType & { id: string }) => {
