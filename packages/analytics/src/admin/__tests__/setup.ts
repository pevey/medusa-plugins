...
import type { AdminRubric, AdminEvent, AdminFunnel } from '../types/analytics'
...
export function makeRubric(overrides: Partial<AdminRubric> = {}): AdminRubric {
	return {
		id: 'arubric_1',
		name: 'product_viewed',
...
export function makeEvent(overrides: Partial<AdminEvent> = {}): AdminEvent {
	return {
		id: 'aevent_1',
...
export function makeFunnel(overrides: Partial<AdminFunnel> = {}): AdminFunnel {
	return {
		id: 'afunnel_1',
		name: 'main_funnel',
		label: 'Main Conversion Funnel',
		description: null,
		steps: ['cart_created', 'order_placed'],
		sales_channel_id: null,
		is_default: true,
		created_at: '2026-01-15T10:00:00.000Z',
		updated_at: '2026-01-15T10:00:00.000Z',
		...
		sdk: {
			client: {
				fetch: (path: string, options?: Parameters<ContractFake['fetch']>[1]) => {
					if (!currentFake) throw new Error('[analytics tests] no fake installed for this test')
