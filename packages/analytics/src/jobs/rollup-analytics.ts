...
export default async function rollupAnalyticsJob(container: MedusaContainer) {
	...
	try {
		...
		for (const row of rows) {
			const existing = await privateAnalyticsService.listAnalyticsEventDailies({
				event: row.event,
				date: startOfDay,
			...
			if (existing[0]) {
				await privateAnalyticsService.updateAnalyticsEventDailies({
					id: existing[0].id,
					...data
				} as any)
			} else {
				await privateAnalyticsService.createAnalyticsEventDailies(data as any)
			}
			upserted++
		}
