...
export default async function refreshSegmentsJob(container: MedusaContainer) {
	...
	try {
		...
		for (const segment of segments) {
			...
			const actorIds = await privateAnalyticsService.evaluateSegmentRules(rules, salesChannelId)

			// Delete existing memberships for this segment
			const existing = await privateAnalyticsService.listAnalyticsSegmentMemberships({
