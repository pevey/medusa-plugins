...
export class Migration20260410165043 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "analytics_event" ("id" text not null, "event" text not null, "actor_id" text null, "group_type" text null, "group_id" text null, "properties" jsonb null, "session_id" text null, "source" text check ("source" in ('storefront', 'backend')) not null default 'backend', "sales_channel_id" text null, "timestamp" timestamptz not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "analytics_event_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "analytics_rubric" ("id" text not null, "name" text not null, "label" text not null, "description" text null, "expected_properties" jsonb null, "active" boolean not null default true, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "analytics_rubric_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "analytics_segment" ("id" text not null, "name" text not null, "label" text not null, "description" text null, "rules" jsonb not null, "sales_channel_id" text null, "created_by" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "analytics_segment_pkey" primary key ("id"));`
		)
