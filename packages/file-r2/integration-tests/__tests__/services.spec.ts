...
describe('R2 File Plugin', () => {
	...
	beforeAll(() => {
		fixtureImagePath = process.cwd() + '/integration-tests/__fixtures__/catphoto.jpg'
	...
	it('uploads, reads via signed URL, and then deletes a private file successfully', async () => {
		...
		const signedUrlFile = Buffer.from(await axios.get(signedUrl, { responseType: 'arraybuffer' }).then(r => r.data))
		...
		const { response } = await axios.get(deletedSignedUrl, { responseType: 'arraybuffer' }).catch(e => e)
	...
	it('deletes multiple files in bulk', async () => {
		...
		await r2Service.delete([{ fileKey: cat.key }, { fileKey: cat1.key }, { fileKey: cat2.key }])
	})
	...
	it('deletes a file by URL', async () => {
		...
		await expect(r2Service.getAsBuffer({ fileKey: resp.key, access: 'public' })).rejects.toThrow()
	})
