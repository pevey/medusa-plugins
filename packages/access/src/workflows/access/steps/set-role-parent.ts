...
export const setRoleParentStep = createStep(
	...
	async (data: SetRoleParentStepInput, { container }) => {
		...
		if (!data || data.length === 0) {
			return new StepResponse({ created: [], removedCount: 0 }, allCompensationData)
		}
		...
		return new StepResponse({ created, removedCount: allToRemoveIds.length }, allCompensationData)
	},
	async (compensationData: Array<{ role_id: string; previous_inherited_role_ids: string[] }> | undefined, { container }) => {
