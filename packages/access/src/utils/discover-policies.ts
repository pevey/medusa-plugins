 ...
 */
export async function discoverPoliciesFromDir(sourcePath?: string, maxDepth: number = 2): Promise<void> {
	if (!sourcePath) {
		return
	}
	...
	const policyDirs = allEntries.filter(e => e.isDirectory() && e.name === 'access-policies').map(e => join((e as any).path as string, e.name))
		...
		policyDirs.map(async scanDir => {
			...
			await Promise.all(
				entries.map(async entry => {
					...
					if (excludedExtensions.some(ext => entry.name.endsWith(ext)) || excludedFiles.includes(entry.name)) {
						return
					}
					...
					if (!hasPolicies) {
						console.warn(`File ${entry.name} in access-policies directory does not export any policies`)
					}
				})
			)
		})
