...
>): Promise<void> => {
  const accessRoleService = container.resolve(
    "accessRoleService"
  ) as ModulesSdkTypes.IMedusaInternalService<InferEntityType<typeof AccessRole>>

  const accessPolicyService = container.resolve(
    "accessPolicyService"
  ) as ModulesSdkTypes.IMedusaInternalService<
    InferEntityType<typeof AccessPolicy>
  >

  const accessRolePolicyService = container.resolve(
