...
const resolvedModulePath = join(
  __dirname,
  "../../../../.medusa/server/src/modules/access"
)
const moduleModels = [
  ...
  testSuite: ({ service }) => {
    describe("access module service", () => {
      ...
      it("rejects a self-parent and a cycle", async () => {
        ...
        await expect(
          service.createAccessRoleParents([{ role_id: a.id, parent_id: a.id }])
        ).rejects.toThrow()

        // a's parent is b; making b's parent a would create a cycle
        await service.createAccessRoleParents([
          { role_id: a.id, parent_id: b.id },
        ])
        await expect(
          service.createAccessRoleParents([{ role_id: b.id, parent_id: a.id }])
        ).rejects.toThrow()
      })
    })
  },
