...
const AccessPolicy = model
	.define('access_policy', {
		id: model.id({ prefix: 'acpol' }).primaryKey(),
	...
	.indexes([
		...
		{ on: ['resource'], where: 'deleted_at IS NULL' },
		{ on: ['operation'], where: 'deleted_at IS NULL' }
	])
