...
import { createCampaignsWorkflow, updateCampaignsWorkflow, updatePromotionsWorkflow } from '@medusajs/medusa/core-flows'
...
export const updateAffiliatePromotionStep = createStep('update-affiliate-promotion-step', async (input: UpdateAffiliatePromotionInput, { container }) => {
	const query = container.resolve(ContainerRegistrationKeys.QUERY)
	const { data } = await query.graph({
		entity: 'promotion',
		fields: ['id', 'code', 'campaign.id', 'campaign.ends_at', 'affiliate.id', 'application_method.value'],
...
export const updateAffiliatePromotionWorkflow = createWorkflow(updateAffiliatePromotionWorkflowId, (input: UpdateAffiliatePromotionInput) => {
				...
				promotionsData: [
					{
						...
						code: input.code,
						application_method: input.discount_value !== undefined ? ({ value: input.discount_value } as any) : undefined
					} as any
				]
			}
		})
	...
	const linkCampaignInput = transform({ input, newCampaigns }, ({ input, newCampaigns }) => ({
		promotionsData: newCampaigns.length ? [{ id: input.promotion_id, campaign_id: newCampaigns[0].id } as any] : []
	}))
