...
export const recalculateStep = createStep('recalculate-affiliate-attributions-step', async (input: RecalculateInput, { container }) => {
	...
	if (!affiliate) {
		throw new MedusaError(MedusaError.Types.NOT_FOUND, `Affiliate ${input.affiliate_id} not found`)
	}
...
export const recalculateAffiliateAttributionsWorkflow = createWorkflow(recalculateAffiliateAttributionsWorkflowId, (input: RecalculateInput) => {
