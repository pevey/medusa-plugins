...
export const addAffiliateAddressStep = createStep(
	...
	async (input: AddAffiliateAddressInput, { container }) => {
		...
		const [created] = await svc.createAffiliateAddresses([
			{ affiliate_id: input.affiliate_id, ...input.address }
		])
...
export const addAffiliateAddressWorkflow = createWorkflow(
	addAffiliateAddressWorkflowId,
	(input: AddAffiliateAddressInput) => {
