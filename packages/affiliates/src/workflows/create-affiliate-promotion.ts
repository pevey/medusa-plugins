...
import { createCampaignsWorkflow, createPromotionsWorkflow, createRemoteLinkStep } from '@medusajs/medusa/core-flows'
...
export const ensureCodeUnclaimedStep = createStep('ensure-promotion-code-unclaimed-step', async (input: { code: string }, { container }) => {
	...
	if (claimed) {
		throw new MedusaError(MedusaError.Types.DUPLICATE_ERROR, `Promotion code ${input.code} is already claimed by an affiliate.`)
	}
...
export const createAffiliatePromotionWorkflow = createWorkflow(createAffiliatePromotionWorkflowId, (input: CreateAffiliatePromotionInput) => {
