	...
	return (
		<Container className="divide-y p-0">
			...
			{imgCollections.length === 0 ? (
				<Text className="text-ui-fg-muted px-6 py-8 text-center">No image content collections found.</Text>
			) : isLoading ? (
				<Text className="text-ui-fg-muted px-6 py-4">Loading...</Text>
			) : items.length === 0 ? (
				<Text className="text-ui-fg-muted px-6 py-8 text-center">{search ? 'No images match your search.' : 'No images in this library.'}</Text>
			) : (
				<div className="flex flex-wrap gap-4 p-6">
					{items.map(item => (
						<div
							key={item.id}
							className="group border-ui-border-base bg-ui-bg-subtle flex flex-none cursor-pointer flex-col overflow-hidden rounded-lg border"
			...
			{pageCount > 1 && (
				<div className="flex items-center justify-between px-6 py-3">
					...
					<div className="flex items-center gap-1">
						<Button size="small" variant="transparent" disabled={pageIndex === 0} onClick={() => setPageIndex(p => p - 1)}>
							<ChevronLeft />
						</Button>
						<Text size="small" className="text-ui-fg-subtle min-w-[4rem] text-center">
							{pageIndex + 1} / {pageCount}
						</Text>
						<Button size="small" variant="transparent" disabled={pageIndex >= pageCount - 1} onClick={() => setPageIndex(p => p + 1)}>
							<ChevronRight />
						</Button>
					</div>
				</div>
