...
export class Migration20260411221609 extends Migration {
	override async up(): Promise<void> {
		...
		this.addSql(
			`create table if not exists "content_creator" ("id" text not null, "name" text not null, "bio" text null, "avatar_url" text null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_creator_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "content_field" ("id" text not null, "name" text not null, "label" text not null, "field_type" text not null, "required" boolean not null default false, "options" jsonb null, "default_value" jsonb null, "sort_order" integer not null default 0, "content_collection_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_field_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "content_item" ("id" text not null, "title" text not null, "slug" text not null, "body" text null, "status" text check ("status" in ('draft', 'published', 'archived')) not null default 'draft', "published_at" timestamptz null, "metadata" jsonb null, "content_collection_id" text not null, "creator_id" text null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_item_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "content_relationship" ("id" text not null, "relationship_type" text check ("relationship_type" in ('many_to_many', 'one_to_many', 'many_to_one')) not null, "source_collection_id" text not null, "target_collection_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_relationship_pkey" primary key ("id"));`
		)
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_content_relationship_source_collection_id" ON "content_relationship" ("source_collection_id") WHERE deleted_at IS NULL;`
		)
		this.addSql(
			`CREATE INDEX IF NOT EXISTS "IDX_content_relationship_target_collection_id" ON "content_relationship" ("target_collection_id") WHERE deleted_at IS NULL;`
		)
		...
		this.addSql(
			`create table if not exists "content_tag" ("id" text not null, "value" text not null, "metadata" jsonb null, "item_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "content_tag_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`create table if not exists "customer_activity" ("id" text not null, "type" text check ("type" in ('post_view', 'search', 'add_to_cart', 'remove_from_cart', 'order_placed', 'order_canceled', 'order_returned')) not null, "user_id" text not null, "metadata" jsonb null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "customer_activity_pkey" primary key ("id"));`
		)
		...
		this.addSql(
			`alter table if exists "content_creator_activity" add constraint "content_creator_activity_creator_id_foreign" foreign key ("creator_id") references "content_creator" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "content_field" drop constraint if exists "content_field_content_collection_id_foreign";`)
		this.addSql(
			`alter table if exists "content_field" add constraint "content_field_content_collection_id_foreign" foreign key ("content_collection_id") references "content_collection" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "content_item" drop constraint if exists "content_item_content_collection_id_foreign";`)
		this.addSql(
			`alter table if exists "content_item" add constraint "content_item_content_collection_id_foreign" foreign key ("content_collection_id") references "content_collection" ("id") on update cascade on delete cascade;`
		)
		this.addSql(`alter table if exists "content_item" drop constraint if exists "content_item_creator_id_foreign";`)
		this.addSql(
			`alter table if exists "content_item" add constraint "content_item_creator_id_foreign" foreign key ("creator_id") references "content_creator" ("id") on update cascade on delete set null;`
		)

		this.addSql(`alter table if exists "content_item_activity" drop constraint if exists "content_item_activity_item_id_foreign";`)
		this.addSql(
			`alter table if exists "content_item_activity" add constraint "content_item_activity_item_id_foreign" foreign key ("item_id") references "content_item" ("id") on update cascade on delete cascade;`
		)
		...
		this.addSql(
			`alter table if exists "content_relationship" add constraint "content_relationship_source_collection_id_foreign" foreign key ("source_collection_id") references "content_collection" ("id") on update cascade on delete cascade;`
		)
		...
		this.addSql(
			`alter table if exists "content_relationship" add constraint "content_relationship_target_collection_id_foreign" foreign key ("target_collection_id") references "content_collection" ("id") on update cascade on delete cascade;`
		)

		this.addSql(`alter table if exists "content_link" drop constraint if exists "content_link_source_item_id_foreign";`)
		this.addSql(
			`alter table if exists "content_link" add constraint "content_link_source_item_id_foreign" foreign key ("source_item_id") references "content_item" ("id") on update cascade on delete cascade;`
		)
		this.addSql(`alter table if exists "content_link" drop constraint if exists "content_link_target_item_id_foreign";`)
		this.addSql(
			`alter table if exists "content_link" add constraint "content_link_target_item_id_foreign" foreign key ("target_item_id") references "content_item" ("id") on update cascade on delete cascade;`
		)
		this.addSql(`alter table if exists "content_link" drop constraint if exists "content_link_relationship_id_foreign";`)
		this.addSql(
			`alter table if exists "content_link" add constraint "content_link_relationship_id_foreign" foreign key ("relationship_id") references "content_relationship" ("id") on update cascade;`
		)

		this.addSql(`alter table if exists "content_tag" drop constraint if exists "content_tag_item_id_foreign";`)
		this.addSql(
			`alter table if exists "content_tag" add constraint "content_tag_item_id_foreign" foreign key ("item_id") references "content_item" ("id") on update cascade on delete cascade;`
		)
	}
