...
describe('TaxLookupProvider', () => {
	...
	describe('constructor', () => {
		...
		it('throws when dataDirectory does not exist', () => {
			expect(() => createProvider({ dataDirectory: '/nonexistent/path' })).toThrow('dataDirectory does not exist')
		})

		it('loads CSV files from the fixtures directory', () => {
			createProvider({ dataDirectory: fixturesDir })
			expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('loaded'))
		})

		it('warns when a CSV is missing required columns', () => {
			createProvider({ dataDirectory: fixturesDir })
			expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('bad-headers.csv'))
		})
		...
		it('warns when directory has no CSV files', () => {
			...
			expect(mockLogger.warn).toHaveBeenCalledWith(expect.stringContaining('no CSV files found'))
	...
	describe('CSV parsing', () => {
		it('loads the CA tax rates CSV', () => {
			createProvider({ dataDirectory: fixturesDir })
			expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('TAXRATES_ZIP5_CA202604.csv'))
		})

		it('loads the LA tax rates CSV', () => {
			createProvider({ dataDirectory: fixturesDir })
			expect(mockLogger.info).toHaveBeenCalledWith(expect.stringContaining('TAXRATES_ZIP5_LA202604.csv'))
		})
		...
		it('converts decimal rates to percentages', async () => {
			...
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('10001'))
		...
		it('ignores extra columns in CSV files', async () => {
			...
			// 90001 -> 0.105 -> 10.5%
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('90001'))
			expect(result).toHaveLength(1)
		...
		it('loads rates from multiple state CSVs', async () => {
			const provider = createProvider({ dataDirectory: fixturesDir })
			// CA zip
			const caResult = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('90001'))
			expect(caResult[0]).toMatchObject({ rate: 10.5 })
			// LA zip
			const laResult = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('70001'))
	...
	describe('getTaxLines', () => {
		...
		it('returns empty array when no postal code', async () => {
			const result = await provider.getTaxLines(makeItemLines(['item_1']), makeShippingLines(['ship_1']), makeContext(undefined))
			expect(result).toEqual([])
		})

		it('returns rate 0 for unknown zip codes', async () => {
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('00000'))
			expect(result).toHaveLength(1)
		...
		it('truncates postal code to 5 digits', async () => {
			...
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('90001-1234'))
			expect(result).toHaveLength(1)
			expect(result[0]).toMatchObject({ rate: 10.5 })
		})

		it('returns tax lines for items and shipping', async () => {
			const result = await provider.getTaxLines(makeItemLines(['item_1', 'item_2']), makeShippingLines(['ship_1']), makeContext('90002'))
		...
		it('looks up a known CA rate', async () => {
			// 90003 -> 0.0975 -> 9.75%
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('90003'))
			expect(result[0]).toMatchObject({ rate: 9.75, code: 'TAX-90003' })
		})

		it('looks up a known LA rate', async () => {
			// 70002 -> 0.0975 -> 9.75%
			const result = await provider.getTaxLines(makeItemLines(['item_1']), [], makeContext('70002'))
