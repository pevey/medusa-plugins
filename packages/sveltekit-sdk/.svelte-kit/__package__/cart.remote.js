...
export const getCartById = query(v.optional(v.string()), async (cartId) => {
    if (!cartId)
        return null;
    const ctx = requestContext();
    const { cart } = await ctx.client.store.cart.retrieve(cartId, cartRelations, ctx.headers());
...
export const addToCart = command(v.object({
...
export const removeFromCart = command(v.string(), async (lineId) => {
    const ctx = requestContext();
    const cartId = currentCartId();
    if (!cartId)
        return null;
...
export const updateCartItem = command(v.object({
...
export const updateCart = command(v.object({
...
export const selectShippingOption = command(v.string(), async (optionId) => {
    const ctx = requestContext();
    const cartId = currentCartId();
    if (!cartId)
        return null;
