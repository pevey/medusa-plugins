...
export function createStoreReviewResource(client: Client) {
	return {
		list: async (productId: string, query?: Record<string, unknown>, headers?: ClientHeaders) => {
			return client.fetch<StoreReviewListResponse>(`/store/reviews/${productId}`, {
				query,
				headers
			})
		},

		create: async (productId: string, body: StoreCreateReviewInput, headers?: ClientHeaders) => {
			return client.fetch<StoreReviewResponse>(`/store/reviews/${productId}`, {
				method: 'POST',
				body,
				headers
			})
		...
		update: async (productId: string, reviewId: string, body: StoreUpdateReviewInput, headers?: ClientHeaders) => {
			return client.fetch<StoreReviewResponse>(`/store/reviews/${productId}/${reviewId}`, {
				method: 'POST',
				body,
				headers
			})
		...
		delete: async (productId: string, reviewId: string, headers?: ClientHeaders) => {
			return client.fetch<StoreReviewDeleteResponse>(`/store/reviews/${productId}/${reviewId}`, {
				method: 'DELETE',
				headers
			})
		},
		...
		summary: async (productId: string, headers?: ClientHeaders) => {
			return client.fetch<StoreReviewSummaryResponse>(`/store/reviews/${productId}/summary`, {
				headers
			})
		},

		listMine: async (query?: Record<string, unknown>, headers?: ClientHeaders) => {
			return client.fetch<StoreMyReviewListResponse>(`/store/customers/me/reviews`, {
				query,
				headers
			})
		}
