...
import { useUpdateWebhookAction, useUpsertWebhookActionQuery, useDeleteWebhookActionQuery, useWebhookSecrets, useWebhookActionQuery } from '../hooks/webhooks'
...
export const EditWebhookActionDrawer = ({
	action,
	triggerId,
	...
	const { mutateAsync: updateAction, isPending } = useUpdateWebhookAction(triggerId, action.id)
	const { mutateAsync: upsertQuery } = useUpsertWebhookActionQuery(triggerId, action.id)
	const { mutateAsync: deleteQuery } = useDeleteWebhookActionQuery(triggerId, action.id)
	...
	const { data: existingQueryData } = useWebhookActionQuery(triggerId, action.id, open && !!action.id && isMedusaEvent)
