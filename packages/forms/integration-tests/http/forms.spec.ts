	...
	testSuite: ({ api, getContainer, dbUtils, utils }) => {
		...
		describe('Validation', () => {
			it('POST /admin/forms rejects missing name', async () => {
				const res = await api.post('/admin/forms', { handle: 'test-handle' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/forms rejects missing handle', async () => {
				const res = await api.post('/admin/forms', { name: 'Test' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/forms rejects invalid handle characters', async () => {
				const res = await api.post('/admin/forms', { name: 'Test', handle: 'UPPERCASE' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('DELETE /admin/forms rejects empty ids array', async () => {
				const res = await api.delete('/admin/forms', { data: { ids: [] }, ...auth() }).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/forms/:id/fields rejects missing name', async () => {
				const res = await api.post('/admin/forms/fake/fields', { label: 'L', field_type: 'text' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/forms/:id/fields rejects missing label', async () => {
				const res = await api.post('/admin/forms/fake/fields', { name: 'n', field_type: 'text' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/forms/:id/fields rejects missing field_type', async () => {
				const res = await api.post('/admin/forms/fake/fields', { name: 'n', label: 'L' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('DELETE /admin/form-submissions rejects empty ids array', async () => {
				const res = await api.delete('/admin/form-submissions', { data: { ids: [] }, ...auth() }).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('POST /admin/form-submissions/:id rejects invalid status', async () => {
				const res = await api.post('/admin/form-submissions/fake', { status: 'invalid' }, auth()).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})
		})
		...
		describe('Forms', () => {
			...
			afterAll(async () => {
				await api.delete('/admin/forms', { data: { ids: [formId, formId2] }, ...auth() }).catch(() => {})
			})
			...
			it('POST /admin/forms/:id updates a form', async () => {
				const res = await api.post(`/admin/forms/${formId2}`, { name: `Newsletter Updated ${ts}` }, auth())
				expect(res.status).toBe(200)
			...
			it('POST /admin/forms creates a form with inline fields', async () => {
				...
				expect(detail.data.form.form_fields.map((f: any) => f.name)).toEqual(expect.arrayContaining(['first_name', 'email']))
		...
		describe('Fields', () => {
			...
			it('POST /admin/forms/:id/fields/:fieldId updates a field', async () => {
				const res = await api.post(`/admin/forms/${formId}/fields/${fieldId}`, { label: 'Your Message', required: false }, auth())
				expect(res.status).toBe(200)
			...
			it('DELETE /admin/forms/:id/fields bulk deletes fields', async () => {
				...
				const res = await api.delete(`/admin/forms/${formId}/fields`, {
					data: { ids },
					...auth()
				})
				expect(res.status).toBe(200)
			...
			it('POST /admin/forms/:id/fields/:fieldId syncs field_options (add, update, remove)', async () => {
				...
				// Add two options
				const syncRes = await api.post(
					`/admin/forms/${formId}/fields/${selectFieldId}`,
					{
						field_options: [
		...
		describe('Field Options', () => {
			...
			beforeAll(async () => {
				...
				const optRes = await api.post(
					`/admin/forms/${formId}/fields/${fieldId}/options`,
					{
						label: 'Small',
						value: 'small',
						sort_order: 0
			...
			it('POST /admin/forms/:id/fields/:fieldId/options/:optionId updates an option', async () => {
				const res = await api.post(`/admin/forms/${formId}/fields/${fieldId}/options/${optionId}`, { label: 'Small (S)' }, auth())
				expect(res.status).toBe(200)
		...
		describe('Form Submissions', () => {
			...
			beforeAll(async () => {
				const formRes = await api.post(
					'/admin/forms',
					{
						...
						active: true,
						turnstile_enabled: false,
						form_fields: [{ name: 'message', label: 'Message', field_type: 'textarea', required: false }]
					},
					auth()
				)
				formId = formRes.data.form.id
			...
			it('POST /admin/form-submissions/:id updates status', async () => {
				const res = await api.post(`/admin/form-submissions/${submissionId}`, { status: 'read' }, auth())
				expect(res.status).toBe(200)
				expect(res.data.form_submission.status).toBe('read')
			})

			it('POST /admin/form-submissions/:id updates status to archived', async () => {
				const res = await api.post(`/admin/form-submissions/${submissionId}`, { status: 'archived' }, auth())
				expect(res.status).toBe(200)
		...
		describe('Turnstile validation', () => {
			...
			it('returns 400 when token is missing', async () => {
				const res = await api.post(`/forms/${formHandle}`, { data: {} }).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})
		...
		describe('Store — POST /forms/:handle', () => {
			...
			it('returns 404 for unknown handle', async () => {
				const res = await api.post('/forms/does-not-exist', { data: {} }).catch((e: any) => e.response)
				expect(res.status).toBe(404)
			})

			it('returns 400 for inactive form', async () => {
				const res = await api.post(`/forms/${inactiveFormHandle}`, { data: {} }).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})

			it('returns 400 when required field is missing', async () => {
				const res = await api.post(`/forms/${requiredFieldFormHandle}`, { data: {} }).catch((e: any) => e.response)
				expect(res.status).toBe(400)
			})
