...
export const GET = async (req: AuthenticatedMedusaRequest<AdminGetFormSubmissionType>, res: MedusaResponse) => {
	const query = req.scope.resolve(ContainerRegistrationKeys.QUERY)

	const {
		data: [form_submission]
	} = await query.graph({ entity: 'form_submission', ...req.queryConfig, filters: { id: req.params.id } }, { throwIfKeyNotFound: true })

	res.json({ form_submission })
}

export const POST = async (req: AuthenticatedMedusaRequest<AdminUpdateFormSubmissionType>, res: MedusaResponse) => {
