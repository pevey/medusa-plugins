...
export const ENUMERABLE_FIELD_TYPES = new Set(['select', 'multiselect'])
...
export const FormFieldsEditor = ({ fields, onChange }: Props) => {
	useBlocker(() => {
		if (fields.length > 0) return !window.confirm('You have unsaved changes. Leave anyway?')
		return false
	})
	...
	const update = (key: string, patch: Partial<FormFieldInput>) => {
